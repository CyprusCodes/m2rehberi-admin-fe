...
import { fetchTags, deleteTag, type Tag as ApiTag } from "@/services/tags";
...
interface Tag {
  ...
  parentId?: string;
  children?: Tag[];
  originalTag: ApiTag; // Store original API tag for editing
}
...
export function TagsTable({ searchTerm, onRefresh }: TagsTableProps) {
  ...
  const [deletingTag, setDeletingTag] = useState<ApiTag | null>(null);
  ...
  const buildHierarchy = (apiTags: ApiTag[]): Tag[] => {
    ...
    apiTags.forEach((apiTag) => {
      const tag: Tag = {
        ...
        isActive: apiTag.isActive === true || apiTag.isActive === 1,
  ...
  const handleEdit = (tag: Tag) => {
    setEditingTag(tag.originalTag);
    setShowEditDialog(true);
  };

  const handleDeleteClick = (tag: Tag) => {
    setDeletingTag(tag.originalTag);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.error || "Tag silinemedi");
    } finally {
      ...
      <React.Fragment key={`tag-fragment-${tag.id}-${level}`}>
        ...
        >
          ...
          <TableCell className="text-white/80">
            {tag.topicsCount.toLocaleString()}
          </TableCell>
          <TableCell>
            <Badge
        ...
        {hasChildren &&
          isExpanded &&
          tag.children!.map((child, index) => renderTagRow(child, level + 1))}
      </React.Fragment>
    );
  };
    ...
    return (
      <div className="rounded-md border border-white/20 bg-white/5 p-8">
        <div className="text-center text-white/70">
          ...
          <p className="text-sm">
            İlk tag'i oluşturmak için "Add Tag" butonunu kullanın.
          </p>
        </div>
      </div>
    );
        ...
        <AlertDialogContent className="bg-gray-900 border-white/20">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
              Tag'i silmek istediğinize emin misiniz?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-white/70">
              Bu işlem geri alınamaz. "{deletingTag?.name}" tag'i kalıcı olarak
              silinecektir.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
