...
interface AddTagDialogProps {
  ...
  onSuccess?: () => void;
  tagToEdit?: ApiTag | null; // Optional prop for editing
}
...
export function AddTagDialog({
  ...
  // Reset or fill form when dialog opens or tagToEdit changes
  useEffect(() => {
    if (open) {
      loadParentTags();
      if (tagToEdit) {
        setFormData({
          ...
          color: tagToEdit.color || "blue",
          parentId: tagToEdit.parentId ? tagToEdit.parentId.toString() : "none",
        });
      } else {
        setFormData({
          name: "",
  ...
  const loadParentTags = async () => {
    try {
      ...
      // Only show parent tags (not children) AND exclude self if editing
      const parents = response.data.filter(
        (tag) =>
          !tag.parentId && (tagToEdit ? tag.tagId !== tagToEdit.tagId : true)
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        parentId:
          formData.parentId === "none" ? undefined : Number(formData.parentId),
      };

      if (tagToEdit) {
        await updateTag(tagToEdit.tagId, payload);
        toast.success(`${formData.name} tag'i başarıyla güncellendi! 🎉`);
      } else {
      ...
      console.error(
        tagToEdit ? "Tag güncelleme hatası:" : "Tag oluşturma hatası:",
        error
      );
      const errorMessage =
        error?.response?.data?.error ||
        (tagToEdit ? "Tag güncellenemedi" : "Tag oluşturulamadı");
      ...
      <DialogContent className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white max-w-2xl mx-auto shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-4">
                  ...
                  >
                    <SelectTrigger className="bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white focus:border-blue-500 focus:ring-blue-500/20">
                      <SelectValue
                        placeholder={
                          loadingParents
                            ? "Parent tag'ler yükleniyor..."
                            : "Ana kategori seçin (isteğe bağlı)"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
                      ...
                      {parentTags.map((tag) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <Badge
                              className={getColorPreview(tag.color)}
                              variant="outline"
                            >
                              {tag.name}
                            </Badge>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {/* Live Preview */}
                    <Badge
                      className={getColorPreview(formData.color)}
                      variant="outline"
                    >
          ...
          <DialogFooter className="gap-3 pt-4">
            ...
            <Button
              type="submit"
              className={`transition-all shadow-lg text-white ${
                isEditMode
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {isEditMode ? "Tag Güncelle" : "Tag Oluştur"}
                </>
              )}
            </Button>
          </DialogFooter>
